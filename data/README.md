...
Each day can also have `solarEvents`, with at most one `sunrise` and one `sunset`. Each event has a `kind`, an `at` timestamp with an explicit UTC offset, and `text` preserving the readable estimate and clock guidance. Keep these consistent with activity `solarNote` values. Friday's 19:20–19:30 sunset estimate uses its midpoint, 19:25. The renderer positions each emoji proportionally within the scheduled activity, using the same timeline geometry as the now indicator. Events outside scheduled activities get a separate timeline row with their time. Hover or focus an interpolated emoji to read its estimate.
