...
function worker(version, stores, options = {}) {
    ...
    vm.runInNewContext(template.replace('__ASSETS__',JSON.stringify(assets)).replace('__VERSION__',version), {
        self: {
            ...
            clients: {
                ...
                async matchAll() {
                    return (options.clientUrls || []).map(url => ({
                        url,
                        async navigate(destination) { navigated.push(destination); }
                    }));
                }
            },
    ...
    return {dispatch, navigated, get claimed(){return claimed;},get skipped(){return skipped;}};
}
