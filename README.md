...
A completed download displays **Ready offline**. When opened as an installed app, it requests persistent storage. Only a confirmed `navigator.storage.persisted()` result adds **Storage protected** to that status. Denied, unsupported, or failed requests leave the ordinary **Ready offline** status. Check this on the iPhone itself; a desktop result does not confirm the phone’s storage protection. Protection reduces automatic eviction risk but does not prevent you from clearing website data. If saving fails, reconnect and tap **Retry**. The app checks for new versions when it launches and whenever it returns to the foreground; tap **Update & reload** when offered. A failed update keeps the existing saved version. Clearing website storage, or storage eviction by the phone, removes the offline copy. Open online again to restore it. Checklist selections stay in browser storage when available; they do not sync between browsers or devices.
...
Rebuild the standalone HTML when editing and include it and the generated `data/itinerary.md` with the source changes. The builder generates this Markdown reference for agents from the same JSON. Below the raw JSON link, the page links to a minimal HTML agent reference at `itinerary-for-agents.html`. The website build generates this page from the same content renderer and includes it in the offline cache. It contains semantic HTML and a small stylesheet, with no JavaScript or embedded images. To generate only Markdown, run `python3 scripts/render_markdown.py`; CI checks freshness with `python3 scripts/render_markdown.py --check` before publishing. The workflow rebuilds the published website from source. Changes to page content, app assets, or the worker generate a new offline cache version. Asset hashes prevent incomplete deployments from replacing a working offline copy.
